...
if __name__ == '__main__':
    ...
    db = redis.StrictRedis(host="localhost", port=6379, db=0, password="admin")
    i = 0
    print("Pose Estimation Waiting for picture input ......")
    while True:
        i = i + 1
        q = db.blpop("image_queue_to_pose_estimation", timeout=0)
        ...
        #     imageRuselt = TfPoseEstimator.draw_humans(img_np, humans, imgcopy=False)  #mat格式
            ...
            # db.hset(imageID, "poseResult", "")


            taskDone = db.hget(imageID, "taskDone")
            if taskDone is None:
            ...
            if taskDone == taskList:
                ...
                # db.rpush("image_queue_tasklist_done", json.dumps(imageData)) 暂时关闭
                db.publish("yanchuangChannel",json.dumps(imageData)) # 测试
            else:
